...
static constexpr int NEIGHBOR_DIST = 6;
...
static void ensure_tables()
{
    ...
    for (int i = 0; i < ARR; i++)
        ...
        {
            int q = i - OFF, r = j - OFF;
            uint64_t base = static_cast<uint64_t>(static_cast<uint32_t>(q)) << 32 | static_cast<uint64_t>(static_cast<uint32_t>(r));
...
>>>>>>> 6402fb8e6c3093d72b970822bdf61c7e000deee9
        ...
        MoveResult get_move(const GameState &gs)
        {
            ...
            for (Coord c : _board_cells)
            {
                ...
                for (const auto &wo : g_win_offsets)
                {
                    ...
                    if (ac || bc)
                    {
                        counts = {static_cast<int8_t>(ac), static_cast<int8_t>(bc)};
                        if (ac >= 4)
                            _hot_a.insert(wo.d_idx, sqi, sri);
                        if (bc >= 4)
                            _hot_b.insert(wo.d_idx, sqi, sri);
                    }
                }
            }
            ...
            if (!_saved)
                _saved = std::make_unique<SavedArrays>();
            ...
            for (int depth = 1; depth <= max_depth; depth++)
            {
                ...
                {
                    int nb4 = _nodes;
                    auto root_result = _search_root_aspiration(turns, depth);
                    ...
                    std::sort(turns.begin(), turns.end(),
                              ...
                              {
                                  double sa = 0, sb = 0;
                                  auto ia = scores.find(a);
                                  if (ia != scores.end())
                                      sa = ia->second;
                                  auto ib = scores.find(b);
                                  if (ib != scores.end())
                                      sb = ib->second;
                                  return maximizing ? (sa > sb) : (sa < sb);
                              });
                    if (std::abs(last_score) >= WIN_THRESHOLD)
                        break;
                }
                catch (const TimeUp &)
                {
        ...
        bool has_near_threats() const
        {
            ...
            for (int d = 0; d < 3; d++)
                for (int qi = 0; qi < ARR; qi++)
                    ...
                    {
                        auto &c = _wc[d][qi][ri];
                        if (c.first >= 3 && c.second == 0)
                            a3++;
                        if (c.second >= 3 && c.first == 0)
                            b3++;
                    }
            return a3 >= 2 || b3 >= 2;
        }
    ...
    private:
        ...
        {
            return _hash ^ (static_cast<uint64_t>(_cur_player) * 0x9e3779b97f4a7c15ULL) ^ (static_cast<uint64_t>(_moves_left) * 0x517cc1b727220a95ULL);
        }
        ...
        void _make(int q, int r)
        {
            ...
            if (player == P_A)
            {
                for (const auto &wo : g_win_offsets)
                {
                    ...
                    if (counts.first >= 4)
                        _hot_a.insert(wo.d_idx, sqi, sri);
                    if (counts.first == WIN_LENGTH && counts.second == 0)
                        won = true;
                }
            }
            else
            {
                for (const auto &wo : g_win_offsets)
                {
                    ...
                    if (counts.second >= 4)
                        _hot_b.insert(wo.d_idx, sqi, sri);
                    if (counts.second == WIN_LENGTH && counts.first == 0)
                        won = true;
                }
            }
            ...
            for (const auto &he : hot.vec)
            {
                ...
                if (my_count >= WIN_LENGTH - 2 && opp_count == 0)
                {
                    ...
                    if (n == 1)
                    {
                        ...
                        return {true, {coord_min(cells[0], other), coord_max(cells[0], other)}};
                    }
                    if (n == 2)
                    {
                        return {true, {coord_min(cells[0], cells[1]), coord_max(cells[0], cells[1])}};
                    }
                }
            }
            return {false, {}};
...
>>>>>>> 6402fb8e6c3093d72b970822bdf61c7e000deee9
            ...
            const flat_set<Coord> &opp_threats)
        {
            ...
            std::vector<Coord> opp_cells, my_cells;
            for (Coord c : opp_threats)
                if (_cand_set.count(c))
                    opp_cells.push_back(c);
            for (Coord c : my_threats)
                if (_cand_set.count(c))
            ...
            if (primary->size() >= 2)
            {
                ...
                std::sort(pairs.begin(), pairs.end(),
                          [&](const Turn &a, const Turn &b)
                          {
                              double da = _move_delta(pack_q(a.first), pack_r(a.first), is_a) + _move_delta(pack_q(a.second), pack_r(a.second), is_a);
                              double db = _move_delta(pack_q(b.first), pack_r(b.first), is_a) + _move_delta(pack_q(b.second), pack_r(b.second), is_a);
                              return maximizing ? (da > db) : (da < db);
                          });
                return pairs;
            }
...
>>>>>>> 6402fb8e6c3093d72b970822bdf61c7e000deee9
        ...
        _search_root(std::vector<Turn> &turns, int depth, double alpha, double beta)
        {
            ...
            for (const auto &turn : turns)
            {
                ...
                if (maximizing && sc > alpha)
                {
                    alpha = sc;
                    best = turn;
                }
                if (!maximizing && sc < beta)
                {
                    beta = sc;
                    best = turn;
                }
...
<<<<<<< HEAD
            ...
            {
                return _search_root(turns, depth, -INF_SCORE, INF_SCORE);
            }
            // loads previous estimate if one exists, otherwise does a normal search with infinite window

            _in_aspiration = true;
            auto root_result = _search_root(turns, depth, alpha, beta);
            double sc = root_result.second[root_result.first];
            while (sc <= alpha || sc >= beta)
            {
                if (sc <= alpha)
                {
                    alpha -= nwindow;
                ...
                root_result = _search_root(turns, depth, alpha, beta);
                sc = root_result.second[root_result.first];
            }
            _in_aspiration = false;
            _tt[_tt_key()] = {depth, _tt_store(sc), TT_EXACT, root_result.first, true};
            return root_result;
        }
...
>>>>>>> 6402fb8e6c3093d72b970822bdf61c7e000deee9
                    ...
                    {
                        flat_set<Coord> all_cells;
                        for (const auto &s : must_hit)
                            all_cells.insert(s.begin(), s.end());
                        bool can_block = false;
                        for (Coord c1 : all_cells)
                        {
                            for (Coord c2 : all_cells)
...
<<<<<<< HEAD
                    ...
                    for (Coord c : cands)
                    {
                        ...
                        if (hi != _history.end())
                            h = static_cast<double>(hi->second);
